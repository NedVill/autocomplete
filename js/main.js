...
function autocomplite(mainInput,arr) {
	...
	function init(mainInput) {
		...
		mainInput.addEventListener("input",function() {
			var val = this.value;
			var count = 0;		
			var dropItems = dropdownWrapper.lastElementChild.childNodes;
			var dropList = dropdownWrapper.lastElementChild;
			removeLists(this);
			dropItems.forEach(function(item, i) {
				var elemVal = item.lastElementChild.getAttribute('data-value');
				if(elemVal.substr(0,val.length).toUpperCase() == val.toUpperCase()) {
					dropList.classList.remove('disabled');
					item.classList.remove('disabled');
					count++;
					item.addEventListener('click',function(){
						mainInput.value = this.getElementsByTagName('div')[0].getAttribute('data-value');
						activeElem = mainInput.value;
						removeLists();
					})
				}
				if(val.length == 0) {
			...
			dropdownHeight(count);
			elemInWindow();	
		});
		mainInput.parentNode.addEventListener('click',function(e) {	
			function starts() {
				var count = 0;
				var dropItems = dropdownWrapper.lastElementChild.childNodes;
				var dropList = dropdownWrapper.lastElementChild;
				...
				dropItems.forEach(function(item, i) {
					...
					item.addEventListener('click',function(){
						mainInput.value = this.getElementsByTagName('div')[0].getAttribute('data-value');
						activeElem = mainInput.value;
						mainInput.parentNode.classList.remove('active');
						mainInput.parentNode.classList.remove('focused');
						removeLists();
					})
				})
				dropdownHeight(count);
		...
		function notEvent(){
			document.addEventListener("click", function (e) {
				if(e.target.classList.contains('input-dropdown')) {
					...
					dropdownWrapper.classList.remove('focused');
					removeLists();
					mainInput.value = activeElem;
				}
		    });
	...
	function createList() {
		...
		mainInput.parentNode.appendChild(wrapper);	
		for(var key in arr) {
			var dropItem = document.createElement('div');
			dropItem.setAttribute('class','dropdown-item');	
			dropItem.classList.add('disabled');
			wrapper.classList.add('disabled');
			dropItem.innerHTML = arr[key].title;
			dropItem.innerHTML += '<div class="drop-elem" id="element-'+ arr[key].id +'" data-value="'+ arr[key].title +'"></div>';
	...
	function dropdownHeight(count) {
		var dropList = mainInput.parentNode.lastElementChild;
	...
	function elemInWindow() {
		var height = window.innerHeight;
		var parent = mainInput.parentNode;
...
autocomplite(document.getElementsByClassName('enter-city')[0],arrs);
autocomplite(document.getElementsByClassName('enter-citys')[0],arrs);